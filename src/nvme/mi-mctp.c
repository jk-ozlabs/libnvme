			       ...
			       struct nvme_mi_resp *resp)
{
	...
	if (len < 0) {
		nvme_msg(LOG_ERR, "Failure sending MCTP message: %m\n");
		return len;
	}
	...
	if (len < 0) {
		nvme_msg(LOG_ERR, "Failure receiving MCTP message: %m\n");
		free(rspbuf);
		return len;
	}
...
nvme_mi_ep_t nvme_mi_open_mctp(unsigned int netid, __u8 eid)
{
	struct nvme_mi_transport_mctp *mctp;
